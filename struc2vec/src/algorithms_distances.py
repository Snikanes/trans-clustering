...
def getDegreeLists(g, root, calcUntilLayer):
    ...
    # Initialize deque and append root node to queue
    queue = deque()
    queue.append(root)

    # Mark the degree of root as 1
    vetor_marcacao[root] = 1
    ...
    while queue:
        ...
        # Pop left vertex (consistent with BFS)
        vertex = queue.popleft()
        timeToDepthIncrease -= 1

        # Append the degree of the vertex to l
        l.append(len(g[vertex]))

        # For each neighbor, if it's entry in vetor_marcacao is 0, update to 1
        # then append v to the queue and increase depth
        for v in g[vertex]:
            if(vetor_marcacao[v] == 0):
                vetor_marcacao[v] = 1
                queue.append(v)
        ...
        # We want to go one layer deeper
        if(timeToDepthIncrease == 0):
            
            # Create array for depths and put on the 'listas'
            lp = np.array(l,dtype='float')
            lp = np.sort(lp)
            listas[depth] = lp
            l = deque()

            # Break if we reached the desired depth
            if(calcUntilLayer == depth):
                break
            
            # Increase depth, time to next depth increase
            depth += 1
            timeToDepthIncrease = pendingDepthIncrease
            pendingDepthIncrease = 0
